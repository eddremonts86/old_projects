...
function model() {
    var html = '<div id="map_campismo_id" style="width: 100%!important; height: 300px  !important; float: left;"></div>';
    ...
    var untiled = new OpenLayers.Layer.WMS("Geoserver layers - Untiled", "http://localhost:8080/googlewms/gmx/wms", {
        LAYERS: 'OSMCache',
        STYLES: '',
        format: format
    }, {
...
function close(izq, der) {
    ...
    console.log($('#right-side').children());
...
function OnSelecthotel(){
        map.setCenter(new OpenLayers.LonLat(-81.1560754, 23.1998595),9,false,false);
}
...
$(document).ready(function () {
    ...
    $.getJSON("jq_mainPanel.php", {index: "prov"}, function (data) {
        var pro = '<option>Institucion - Casa Matris - Puntos de Recojida</option> ';
        for (var i = 0; i < data.length; i++) {
            pro += '<option>' + data[i].nombre + '</option>'
        }
        $('#prov').html(pro);
    });
    $.getJSON("jq_mainPanel.php", {index: "tipo"}, function (data) {
        var pro = '<option>Tipos</option> ';
        for (var i = 0; i < data.length; i++) {
            pro += '<option>' + data[i].nombre + '</option>'
        }
        $('#prov1').html(pro);
    ...
    $.getJSON("jq_mainPanel.php", {index: "grafic"}, function (data) {
        ...
        Morris.Area({
            ...
            labels: [ 'America','Nacional', 'Europa'],
            pointSize: 5,
            hideHover: 'auto',
            resize: true
        });
    ...
    $('#dawn_left').click(function () {
        $('#left-side').hide(1000);
        var s = $("#center").find("center_fool_LF").addClass("center_fool");
        console.log(s.selector);
        if(s.selector!='#center center_fool_LF')
            $("#center").addClass("center_fool_LF").show(1000);
    ...
    $('#tip_save').click(function () {
       var tips_inst =  $('#tips_inst').val();
       var save_tipe='save_tipe';
        if(tips_inst!=''||tips_inst!=null){
            $.getJSON("jq_mainPanel.php",{
                index:save_tipe,
                tips_inst:tips_inst
            },
                function(data){
    ...
    /*$('li.acordion, a').mouseenter(function(a){
       $(a.currentTarget.children[1]).show(1100);
        if(a.target == this){}
       *//* $(a.currentTarget.children[1]).show(1100);
        var out = $(a.currentTarget.children[1]).is(function(){$(a.currentTarget.children[1]).mouseout();})
        var out1 = $(a.currentTarget.children[1]).is(function(){$(a.currentTarget.children[0]).mouseout();})
        if(!out && !out1 ){$(a.currentTarget.children[1]).hide(1000);}*//*
    });*/
