...
class Especialidad extends Module {
    ...
    public function CargarDatos(&$params) {
        ...
        $_limit = $params['limit'];
        $nivel = $params['nivel'];
        $_table = $this->_dbase->GetTable('vistas_resumen.especialidades');
    ...
    public function ValidateCargarDatosNIvel(&$params) {
        return true;
    }
    public function CargarDatosNIvel(&$params) {
        $_data = array();
        $_count = 0;
        $_start = ($params['start']) ? $params['start'] : 0;
        $_limit = $params['limit'];
    ...
    public function ValidateEliminar(&$params) {
        ...
        $count = $datos->Contains(array('especialidad_id' => "'$id'"));
        if ($count > 0) {
            $this->RegisterError('Operación no válida', "El Especialidad a eliminar esta siendo usado por Contrato.");
            return false;
        }
        return true;
    }
    public function Eliminar(&$params) {
        ...
        $result=null;
            if ($id) {
                $datos=$this->_dbase->GetTable("nomencladores.especialidades");
    ...
    public function ValidateAdd(&$params){
        ...
        if (!$_valid) {
            $this->RegisterError('Parámetro incorrecto', "Nombre del Especialidad incorrecto.");
            return false;
        }
        ...
        $actividades = $this->_dbase->GetTable('nomencladores.especialidades');
        ...
        elseif ($count > 0) {
            $this->RegisterError('Operación no válida', "El Especialidad '$nombre' ya existe.");
            return false;
        }return true;
       }
    public function Add(&$params){
       $name=$params['nombre_id'];
        $nivel = $params['nivel'];
       $datos=$this->_dbase->GetTable("nomencladores.especialidades");
       return $data=$datos->InsertValues(array('nombre' => "'$name'",'id_nivel' => "'$nivel'"));
    ...
    public function Modid(&$params){
        ...
        $nivel = $params['nivel'];
        $datos=$this->_dbase->GetTable("nomencladores.especialidades");
        $datos->Update(array('nombre' => "'$name'",'id_nivel' => "'$nivel'"),"id = '$ids'");
        return true;
    }
};
?>
