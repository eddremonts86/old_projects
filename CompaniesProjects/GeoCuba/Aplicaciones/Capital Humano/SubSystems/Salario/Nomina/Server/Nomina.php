...
class Nomina extends Module
{
    ...
    public function info(&$params)
    {
        ...
        $mes = $params['mes'];
        $anno = $params['anno'];
        $_table = $this->_dbase->GetTable('vistas_resumen.salario_nomina_resumen');
        $show = $_table->GetAll("mes = '$mes' and  anno ='$anno'");
        $h = 0;
        $p = 0;
        $area = null;
        while ($h < count($show)) {
            if ($area != $show[$h]['area']) {
                ...
                for ($i = 0; $i < count($show); $i++) {
                    $inc=$show[$i]['inc'];if($show[$i]['inc']==null||$show[$i]['inc']=="")$inc=0;
                ...
                $_data[$p]['datos'] .= '<br><table style="border: 0px;width: 100%;margin-left: 20px;margin-right: 20px;margin-top: 5px">
	                                    <tr>
                                        <td>Elaborado por: <b><i><u>'.$show1[0]['n_elab'].'</b></td>
                                        <td>Aprobado por: <b><i><u>'.$show1[0]['n_aprob'].'</b></td>
                                        </tr>
                                        <tr>
                                        <td>Revisado por: <b><i><u>'.$show1[0]['n_realiz'].'</b></td>
                                        <td>Contabilizado por: <b><i><u>'.$show1[0]['n_contab'].'</b></td>
                                        </tr>
                                        </table></div>';
                $area = $show[$h]['area'];
                $p++;
...
//                $result = $datos->DeleteWhere("id = '$idcargo'");
    ...
    public function Add(&$params)
    {
        ...
        $mes = $params['mes'];
        $anno = $params['anno'];
        $nomina = $this->_dbase->GetTable("salario.nomina");
       // print_r($data);
        for ($i = 0; $i < count($data); $i++) {
            $id_prenomina = $data[$i]->id_prenomina;
            $debito  = $data[$i]->debit ;
            $salario  = $data[$i]->total;
            $sub_total  = $data[$i]->subtotal;
            $inc_diner  = $data[$i]->inc_diner;
            if ($id_prenomina) {
                $cant = $nomina->GetRowsCount("mes = '$mes' and  anno ='$anno' and id_prenomina ='$id_prenomina'");
                if($cant==0){
                    $nomina->InsertValues(array('id_prenomina' => "'$id_prenomina'",'debito' => "'$debito'",'inc' => "'$inc_diner'",
                                                'salario' => "'$salario'",'mes' => "'$mes'",'anno' => "'$anno'",'sub_total' => "'$sub_total'"));
                }
                else {
                    $nomina->Update(array('id_prenomina' => "'$id_prenomina'",'debito' => "'$debito'",'salario' => "'$salario'",'inc' => "'$inc_diner'",
                                          'mes' => "'$mes'",'anno' => "'$anno'",'sub_total' => "'$sub_total'"), "id_prenomina = '$id_prenomina'");
                }
            }
        }
        return true;
    ...
    public function Buscar2(&$params)
    {
        $mes = $params['mes'];
        $anno = $params['anno'];
        $_table = $this->_dbase->GetTable('vistas_resumen.salario_nominas_debito');
...
//            $datos = pg_query($sql);
    ...
    public function cobro(&$params)
    {
        ...
        $_limit = 25;
        $_table = $this->_dbase->GetTable('vistas_resumen.salario_nominas_debito');
        $_data = $_table->GetRange($_limit, $_start, "month='$mes' and year='$anno'");
        if (is_null($_data)) return false;
        $_count = $_table->GetRowsCount("month='$mes' and year='$anno'");
