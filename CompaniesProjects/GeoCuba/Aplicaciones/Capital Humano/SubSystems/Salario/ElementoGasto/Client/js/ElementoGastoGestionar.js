function ElementoGastoGestionar()
{
    this._btn_add = null;
    this._btn_mod = null;
    this._btn_del = null;    
    this.Render = function(Panel){
        this._btn_add = Ext.create('Ext.Button',{
            ...
            handler:function(){
                ...
                Ext.getCmp('ElementoGasto_grid_id').store.add(record);
            }
        });
        this._btn_del = Ext.create('Ext.Button',{
            text: 'Eliminar',
            id: 'del_ElementoGasto_btn_id',
        ...
        var tbar = Ext.getCmp('ElementoGasto_tbar_id');
        ...
        tbar.add({
            ...
            listeners: {
                change:function(){
                    console.log(Ext.getCmp('ElementoGasto_grid_id').store);
                    Ext.getCmp('ElementoGasto_grid_id').store.clearFilter();
                    Ext.getCmp('ElementoGasto_grid_id').store.filter("nombre", Ext.getCmp('searchField_class_cien').getValue());
                },scope:this
            }
        });*/
    }
...
App.RegisterFunction('ElementoGastoGestionar', new ElementoGastoGestionar());
this.panel=null;
this.panelupd=null;
ElementoGasto.prototype.OnShowWindowcp = function(_paccion){
    var _value_nom = null;
    if(_paccion == 'add')
    {
...
ElementoGasto.prototype.OnShowWindow2cp = function(_paccion){
    var _value_nom_oll = null;
    var id_tpo=null;
    var _selectionModel = Ext.getCmp('ElementoGasto_grid_id').getSelectionModel();
...
ElementoGasto.prototype.Add = function(_paccion){

    if (Ext.getCmp('panel').getForm().isValid())
    {
        App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
        var   _result = App.PerformSyncServerRequest('ElementoGasto.ElementoGasto.Add',this.panel.getForm().getValues());
...
ElementoGasto.prototype.Modid = function(_paccion,_selectionModel,_selected_rcd){

    if (Ext.getCmp('panelupd').getForm().isValid())
    {
        App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
        var _result = App.PerformSyncServerRequest('ElementoGasto.ElementoGasto.Modid',this.panelupd.getForm().getValues());
        this.__data_store.load();
        App.HideMsgBox();
        ...
        if(_result)
        {
            ...
            this.__data_store.load();
            App.InfoMessage('Informaci&oacute;n', 'ElementoGasto modificado satisfactoriamente');
        }
    }
 }
ElementoGasto.prototype.deltectcp=function(_paccion){
    var me = this;
    var fnCallBack = function() {
        var _selected_rcd = Ext.getCmp('ElementoGasto_grid_id').getSelectionModel().getLastSelected();
        var id = _selected_rcd.data.id;
        App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
        _result = App.PerformSyncServerRequest('ElementoGasto.ElementoGasto.Eliminar', {id: id});
        App.HideMsgBox();

        if(_result)
        {
            me.__data_store.load();
            App.InfoMessage('Informaci&oacute;n', 'ElementoGasto eliminado correctamente');
        }
    }
