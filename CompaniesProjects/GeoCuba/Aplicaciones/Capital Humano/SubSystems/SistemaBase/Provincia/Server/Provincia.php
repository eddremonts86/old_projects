...
class Provincia extends Module {
    ...
    public function ValidateEliminar(&$params) {
        $id=$params['id'];
        $datos=$this->_dbase->GetTable("nomencladores.municipio");
        $count = $datos->Contains(array('provinciaid' => "'$id'"));
        if ($count > 0) {
            $this->RegisterError('Operación no válida', "La Provincia a eliminar está siendo usado por Municipio.");
            return false;
        }
        return true;
    }
    public function Eliminar(&$params) {
        ...
        $result=null;
            if ($id) {
                $datos=$this->_dbase->GetTable("nomencladores.municipio");
                $free=$datos->GetAll("provinciaid= '$id'");
                if(is_null($free)){
                    $this->RegisterError('Parámetro incorrecto', "No se puede eliminar una Provincia que tiene Municipios asignados.");
                    return false;
                }
                else{
                $data=$this->_dbase->GetTable("nomencladores.provincia");
    ...
    public function ValidateAdd(&$params){
        ...
        if (!$_valid) {
            $this->RegisterError('Parámetro incorrecto', "Nombre de la Provincia incorrecta.");
            return false;
        }
        ...
        $actividades = $this->_dbase->GetTable('nomencladores.provincia');
        ...
        elseif ($count > 0) {
            $this->RegisterError('Operación no válida', "La Provincia '$nombre' ya existe.");
            return false;
        }return true;
       }
    public function Add(&$params){
       $name=$params['nombre_id'];
       $datos=$this->_dbase->GetTable("nomencladores.provincia");
