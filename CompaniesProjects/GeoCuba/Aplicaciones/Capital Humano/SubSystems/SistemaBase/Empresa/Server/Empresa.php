...
class Empresa extends Module {
    ...
    public function CargarDatos(&$params) {
        ...
        $_data = $_table->GetRange($_limit, $_start,"empresa.activo = 'true'");
        if (is_null($_data))
            return false;
        $_count = count($_data);
        if ($_count == -1)
    ...
    public function ValidateEliminar(&$params) {
        ...
        $count = $datos->Contains(array('empresaid' => "'$id'"));
        if ($count > 0) {
            $this->RegisterError('Operación no válida', "La Empresa  a eliminar esta siendo usado por Agencia.");
            return false;
        }
        return true;
    }
    public function Eliminar(&$params) {
        ...
        $result=null;
            if ($id) {
                $datos=$this->_dbase->GetTable("nomencladores.empresa");
    ...
    public function ValidateAdd(&$params){
        ...
        if (!$_valid) {
            $this->RegisterError('Parámetro incorrecto', "Nombre de la Empresa incorrecto.");
            return false;
        }
        ...
        $actividades = $this->_dbase->GetTable('nomencladores.empresa');
        ...
        elseif ($count > 0) {
            $this->RegisterError('Operación no válida', "La Empresa '$nombre' ya existe.");
            return false;
        }return true;
       }
