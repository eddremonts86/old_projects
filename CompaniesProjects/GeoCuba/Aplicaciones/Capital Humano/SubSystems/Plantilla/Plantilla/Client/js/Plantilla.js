            ...
            columns: [
                ...
                },
                {
                    ...
                    items: [
                        ...
                        tooltip: 'Ficha Personal',
                        handler: function(grid, rowIndex, colIndex) {
                            var rec = grid.getStore().getAt(rowIndex);
                            var id = rec.get('trabajadorid');
                            _result = App.PerformSyncServerRequest('Plantilla.Plantilla.PrintFicha', {id:id});
                            App.PrintData(null,_result);
                            }
                       },
                       {width:'2px'},
                       {
                        iconCls: 'document_export16',
                        tooltip: 'Movimiento de Nomina',
                        handler: function(grid, rowIndex, colIndex) {
                            var rec = grid.getStore().getAt(rowIndex);
                            var id = rec.get('trabajadorid');
                            _result = App.PerformSyncServerRequest('Plantilla.Plantilla.PrintMovN', {id:id});
                            App.PrintData(null,_result);
                       ...
                       {width:'2px'},
                       {
                            ...
                            handler: function(grid, rowIndex, colIndex) {
                                var rec = grid.getStore().getAt(rowIndex);
                                var id = rec.get('trabajadorid');
                                _result = App.PerformSyncServerRequest('Plantilla.Plantilla.PrintContrato', {id:id});
                                App.PrintData(null,_result);
                            }
                        },
                       {width:'2px'},
                       {
                            iconCls: 'edit_create_symbolic_link16',
                            tooltip: 'Anexo de Contrato',
                            handler: function(grid, rowIndex, colIndex) {
                                var rec = grid.getStore().getAt(rowIndex);
                                var id = rec.get('trabajadorid');
                                _result = App.PerformSyncServerRequest('Plantilla.Plantilla.PrintContratoAnexo', {id:id});
                                App.PrintData(null,_result);
                            }
                        }
