...
final class ConnectionsManager
{
     ...
     public static function GetDatabase($connectionName)
     {
          ...
          else
          {
               ...
               $_config_path = $_SESSION['APP_PATH'] . "/SubSystems/$_subsystem/Config/Server/db_config.php";
     ...
     public static function GetCommonDatabase($connectionName)
     {
          if(isset($_SESSION[$connectionName]))
               $_connection_data = $_SESSION[$connectionName];
          else
          {
               // Buid the path to include the config storage class
               $_config_path = realpath(dirname(__FILE__)) . '../../../../App/Config/common_access.php';
               // Validate that the file exists
               if(!is_file( $_config_path ) )
               {
                    ErrorManager::RegisterError('IO Error', "Not common connections defined");
                    return null;
               }
               ...
               require_once($_config_path);
               // Build the name of the class
               $_class_name = "CommonConnections";
               // Validate that the class exists
               if (!class_exists($_class_name, false))
               {
                    ErrorManager::RegisterError('Class Error', "Not common connections class defined");
                    return null;
               }
               ...
               if (!is_subclass_of($_connections_storage, 'ConnectionsStorage'))
               {
                    ErrorManager::RegisterError('ConnectionsStorage Error', 'Invalid class of common connections storage');
                    return null;
               }
          ...
          if(is_null($_connection_data))
          {
               ErrorManager::RegisterError('Connection error', "The common connection '$connectionName' does not exists");
               return null;
          }
