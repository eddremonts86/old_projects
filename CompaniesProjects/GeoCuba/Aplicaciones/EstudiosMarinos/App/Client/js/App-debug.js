                    ...
                    { html:
                        '<div class="bs-docs-example">'+
                            '<ul id="myTab" class="nav nav-tabs">'+
                                '<li class="active"><a href="#home" data-toggle="tab">Información del Usuario</a></li>'+
                                '<li><a href="#profile" data-toggle="tab">Información General</a></li>'+
                                 '<li><a href="#dropdown1" data-toggle="tab">Contacto</a></li>'+
                            '</ul>'+
                            '<div id="myTabContent" class="tab-content">'+
                                '<div class="tab-pane fade in active" id="home">'+
                                    '<p><img width="70px" height="70px" src="App/Client/img/logo/100.png"/>' +
                                     img()+
                            ...
                            '</p>'+
                                ...
                                '<br>' +
                                '<button class="btn btn-mini" type="button">Enviar</button>'+
                                '</div>'+
                            '</div>'+
                        '</div>'
                    }
...
//            document.getElementById('_working_space_id').innerHTML = '';
    ...
    PrintModel:function(store,title){
        ...
        if(title==""||title==null||title==undefined)
        {
            for(var j = 0; j < itcant; j++){
                if(html == ''){
                    html = "<html><head><link id='default' rel='stylesheet' type='text/css' href='../../../Framework/Client/frk2.0/dist/css/bootstrap-theme.css'>\n\
                ...
                for(var i = 0;i<cant;i++){
                    if(store.data.items[j].fields.items[i].header != undefined){
                        if(store.data.items[j].data[store.data.items[j].fields.items[i].name] != null && store.data.items[j].data[store.data.items[j].fields.items[i].name] != '')
                            ...
                            if(store.data.items[j].data[store.data.items[j].fields.items[i].name] == 0){
                                html += "<td> 0 </td>";
                            }
                            else{
                                html += "<td>&nbsp;</td>";
    ...
    Print_Store_plus_HTML:function(title,store,HTML){
        if(store!=="" &&  store!=null && store!=undefined && HTML!="" && HTML!=null && HTML!=undefined){
            var itcant = store.data.items.length;
            var cant   = store.data.items[0].fields.items.length;
            var html = "";
            if(title==""||title==null||title==undefined)
