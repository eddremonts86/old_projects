...
class Municipio extends Module {
    ...
    public function CargarDatos(&$params) {
        ...
        $_limit = $params['limit'];
        $_table = $this->_dbase->GetTable('nomencladores.municipio');
    ...
    public function ValidateEliminar(&$params) {
        $id=$params['id'];
        $datos=$this->_dbase->GetTable("nomencladores.reparto");
        $count = $datos->Contains(array('municipioid' => "'$id'"));
        if ($count > 0) {
            $this->RegisterError('Operación no válida', "El Municipio a eliminar esta siendo usado por Reparto.");
            return false;
        }
        return true;
    }
    ...
    public function ValidateAdd(&$params){
        ...
        if (!$_valid) {
            $this->RegisterError('Parámetro incorrecto', "Nombre del Municipio incorrecto.");
            return false;
        }
        ...
        $actividades = $this->_dbase->GetTable('nomencladores.municipio');
        ...
        elseif ($count > 0) {
            $this->RegisterError('Operación no válida', "El Municipio '$nombre' ya existe.");
            return false;
        }return true;
       }
    public function Add(&$params){
       ...
       $datos=$this->_dbase->GetTable("nomencladores.municipio");
       return $data=$datos->InsertValues(array('nombre' => "'$name'",'provinciaid' => "'$datos_UBT_id'"));
    ...
    public function Modid(&$params){
        ...
        $datos_UBT_id=$params['datos_UBT_id'];
        $datos=$this->_dbase->GetTable("nomencladores.municipio");
        $datos->Update(array('nombre' => "'$name'",'provinciaid' => "'$datos_UBT_id'"),"id = '$ids'");
        return true;
    }
