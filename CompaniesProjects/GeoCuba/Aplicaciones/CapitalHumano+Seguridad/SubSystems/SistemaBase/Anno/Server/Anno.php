...
class Anno extends Module {
    ...
    public function ValidateEliminar(&$params) {
        ...
        $count = $datos->Contains(array('annoid' => "'$id'"));
        if ($count > 0) {
            $this->RegisterError('Operación no válida', "El Año a eliminar esta siendo usado por Mes.");
            return false;
        }
        ...
        if ($count2 > 0) {
            $this->RegisterError('Operación no válida', "El Año a eliminar esta siendo usado por Año_Mes.");
            return false;
        }
        return true;
    }
    ...
    public function ValidateAdd(&$params){
        ...
        if (!$_valid) {
            $this->RegisterError('Parámetro incorrecto', "Nombre del Año incorrecto.");
            return false;
        }
        ...
        elseif ($count > 0) {
            $this->RegisterError('Operación no válida', "El Año '$nombre' ya existe.");
            return false;
        }return true;
       }
    ...
    public function ValidateModid(&$params){
        ...
        if (!isset($params['nombre_id_up'])||!isset($params['id_Udt_id'])) {
            $this->RegisterError('Parámetro incorrecto', "Parámetro no definido o nulo.");
            return false;
        }
        $nombre = $params['nombre_id_up'];
        $id = $params['id_Udt_id'];
    ...
    public function Modid(&$params){
        $name=$params['nombre_id_up'];
        $ids=$params['id_Udt_id'];
        $datos=$this->_dbase->GetTable("nomencladores.anno");
        $datos->Update(array('valor' => "'$name'"),"valor = '$ids'");
        return true;
    }
