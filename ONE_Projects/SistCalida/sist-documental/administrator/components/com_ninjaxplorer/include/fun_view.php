...
function nx_show_file($dir, $item) {		// show file contents
	...
	if( @eregi($GLOBALS["images_ext"], $item)) {
		echo '<img src="'.$GLOBALS['home_url'].'/'.$dir.'/'.$item.'" alt="'.$GLOBALS["messages"]["actview"].": ".$item.'" /><br /><br />';
			...
			if( file_exists($GLOBALS['JURI::base()'] . '/includes/domit/xml_saxy_shared.php')) {
				require_once( $GLOBALS['JURI::base()'] . '/includes/domit/xml_saxy_shared.php' );
			} elseif( file_exists( $GLOBALS['JURI::base()'] . '/libraries/domit/xml_saxy_shared.php')) {
				require_once($GLOBALS['JURI::base()'] . '/libraries/domit/xml_saxy_shared.php');
			} else {
				return;
			}
			
			if( file_exists($GLOBALS['JURI::base()'] . '/plugins/content/geshi/geshi.php')) {
				$geshiFile = $GLOBALS['JURI::base()'] . '/plugins/content/geshi/geshi.php';
			} elseif(file_exists($GLOBALS['JURI::base()'] . '/libraries/geshi/geshi.php')) {
				$geshiFile = $GLOBALS['JURI::base()'] . '/libraries/geshi/geshi.php';
			}
