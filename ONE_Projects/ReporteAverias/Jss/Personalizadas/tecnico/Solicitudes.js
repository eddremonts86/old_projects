...
var panel;
var Prov, Munc, Dept,formulario,formulario1,formulario2,RPA,individual;
var mask,coment; 
var provin,idR1,idR,idR2;
var municipios;
...
com.quizzpot.tutorial.IntegracionTutorial = {
	init: function(){
		...
		idR1 = new Ext.form.TextField({fieldLabel: 'Numero Reporte',name: 'idr1',anchor:'50%',forceSelection : true,	id : "idr1",emptyText:'59874 '});		
		...
		RPA = new Ext.form.ComboBox({fieldLabel : "Tipo Rotura",anchor:'95%',store:['Hardware','Software'],bodyStyle:'padding:5px 5px 0',name : "RPA",hiddenName : "RPA",id : "rPA",forceSelection : true,
														emptyText:'Selecciona ...',displayField : 'name',typeAhead : true,forceSelection : true,mode : 'local'  });  
		//**********  PROXY COMBOBOX  Equipos*******************************************
	    var  proxycombobox = new Ext.data.HttpProxy({url: '../../../clases/envio.php?termino=Equipos',method:'POST'});
	    var  readercombobox = new Ext.data.JsonReader({totalProperty: 'total', root: 'data',
			fields: [{name: 'id',mapping:'id'},{name: 'nombre',mapping:'nombre'}]});
	    var  equipos = new Ext.data.Store({proxy: proxycombobox,totalProperty: 'total',reader: readercombobox});
	    var equipo = new Ext.form.ComboBox({
					name : "equipo",hiddenName : "equipo",id : "equipo",emptyText:'Seleccione Equios...',
					fieldLabel : "Equipo",anchor:'95%',bodyStyle:'padding:5px 5px 0',					
	    ...
	    var Dept = new Ext.form.ComboBox({
					name : "Dept",hiddenName : "Dept",id : "Dept",emptyText:'Seleccione Departamento...',
					fieldLabel : "Departamento",anchor:'95%',bodyStyle:'padding:5px 5px 0',					
					forceSelection : true,displayField : 'nombre', valueField : 'nombre', 			 
					loadingText : 'Querying....', minChars : 1, triggerAction : 'all',	
					store: areas});
		/*==========Fin============*/		
		...
		var proxy = new Ext.data.HttpProxy({ 
				api: {  
					read    : "../../../clases/envio.php?termino=ReportesHard",   
		...
		var proxy1 = new Ext.data.HttpProxy({ 
				api: {  
					read    : "../../../clases/envio.php?termino=ReportesSoft",   
					create  : "../../../clases/resepcion.php?termino=AltaSolicitud",  
					update  : "../../../clases/resepcion.php?termino=BajaSolicitud",  
					destroy : "../../../clases/resepcion.php?termino=eliminarSolicitud"  
				}  
		...
		var reader1 = new Ext.data.JsonReader({   
			...
			var writer1 = new Ext.data.JsonWriter({   
				encode      : true,  
				writeAllFields  : true  
			})  
			...
			this.store1 = new Ext.data.GroupingStore({  
				proxy       : proxy1,  
				reader      : reader1,  
				writer      : writer1,				
				autoSave    : true ,
			...
			this.store1.load({params:{z:30,start:0,limit:15}});  
			...
			pager1.on('beforechange',function(bar,params){
			params.z = 30;
		});
			this.grid = new Ext.grid.GridPanel({   
					...
					columns : [  							
							...
							{header:'Reporte', dataIndex:'id', width:90,sortable: true,hidden:false},
					...
					{text:'Dar alta Solicitud', scope:this,iconCls:'acept',handler:this.Alta},
					{text:'Pasar a Pendiente', scope:this,iconCls:'user_del',handler:this.Baja},
					{text:"Eliminar Solicitud", scope:this, handler:this.onDelete,iconCls:'del'}, 
							'->',
			...
			this.grid1 = new Ext.grid.GridPanel({   
					...
					tbar  : [   
					{text:"Ver Datos", scope : this, handler: this.datos1,iconCls:'info'},
					{text:'Dar alta Solicitud', scope:this,iconCls:'acept',handler:this.Alta1},
					{text:'Pasar a Pendiente', scope:this,iconCls:'user_del',handler:this.Baja1},
					{text:"Eliminar Solicitud", scope:this, handler:this.onDelete1,iconCls:'del'}, 
							'->',
							new Ext.form.TwinTriggerField({
								xtype: 'twintriggerfield',
								trigger1Class: 'x-form-clear-trigger',
			...
			listado1.on('rowclick',function(Grid, rowIndex, e){filaseleccionada = rowIndex;});		
			this.form = new Ext.form.FormPanel({
						...
						bbar: [{iconCls:'enviar1',text:'Envio',id:'envio',scope:this,handler:this.sendData},
			...
			items:[
				{items:	[this.grid], title: 'Solicitudes de Hardware - Realizadas'},
				{items:	[this.grid1], title: 'Solicitudes de Software - Realizadas'}				
			]
		});
			var center = {
					xtype	:	"panel",
			...
			var west =  {
					...
					collapseMode: 'mini',
					collapsible: true,
					tools: [{id:'pin',handler:this.fuera},
						{id:'unpin',handler:this.dentro}
						],
					split	:	true,					
					frame:true,	
					items	:	[this.form],					 
	...
	onDelete1:function(){
		var rows = listado1.getSelectionModel().getSelections();   
		if(rows.length === 0){  
		 return false;  
		 }  
		this.store1.remove(rows);			
		},
	sendData: function(){
	...
	Alta:function(){		
	idR1.setValue(listado.getStore().getAt(filaseleccionada).get('id')); 		
	coment=new Ext.form.HtmlEditor({fieldLabel : "Comentario Tecnico",anchor:'95%',bodyStyle:'padding:5px 5px 0',name : "Coment",hiddenName : "Coment",height: 150});
	 this.formulario = new Ext.form.FormPanel({
						...
						id:'forma1',
						height:170,
						url: '../../../clases/resepcion.php?termino=DarAlta',						 
						bodyStyle: 'padding:0 10px 0;',
						columnWidth: '.5',
						border: false,
						items:	[idR1,coment]																
			...
			mask = new Ext.Window({
			title: 'Pasar a solicitudes resueltas ',
			layout: 'form',width: 710,frame:true,height:228,
			items:[this.formulario]
			,bbar: ['->',{text:'Enviar Datos', scope:this,iconCls:'acept',handler:this.AltaEnvio},
					{text:'Cancelar Envio', scope:this,iconCls:'user_del',handler:this.reset}]	
			});
			formulario1=this.formulario;
			mask.show(); 
				
		},
	AltaEnvio: function(){
	var mask = new Ext.LoadMask(Ext.get('forma1'),{msg:'Guardando los datos. Por Favor espere...'});
			mask.show(); 
			formulario1.getForm().submit({  
		     	method: 'put',  	 
	...
	failure: function(form,action){  
   	...
   	Baja:function(){
	 idR1.setValue(listado.getStore().getAt(filaseleccionada).get('id')); 		
	comenta=new Ext.form.HtmlEditor({fieldLabel : "Comentario Tecnico",anchor:'95%',bodyStyle:'padding:5px 5px 0',name : "Comenta",hiddenName : "Comenta",height: 150});
	 ...
	 this.formulario = new Ext.form.FormPanel({
						...
						id:'forma2',
						height:170,
						url: '../../../clases/resepcion.php?termino=DarPendiente',						 
						bodyStyle: 'padding:0 10px 0;',
						columnWidth: '.5',
						border: false,
						items:	[idR1,comenta]																
			...
			mask = new Ext.Window({
			title: 'Pasar a solicitudes Pendientes ',
			layout: 'form',width: 710,frame:true,height:228,
			items:[this.formulario]
			,bbar: ['->',{text:'Enviar Datos', scope:this,iconCls:'acept',handler:this.BajaEnvio},
					{text:'Cancelar Envio', scope:this,iconCls:'user_del',handler:this.reset}]	
			});
			formulario1=this.formulario;
			mask.show();  
	...
	BajaEnvio: function(){
	var mask = new Ext.LoadMask(Ext.get('forma2'),{msg:'Guardando los datos. Por Favor espere...'});
			mask.show(); 
			formulario1.getForm().submit({  
		     	method: 'put',  	 
	...
	datos1:function(){
		namebre.setValue(listado1.getStore().getAt(filaseleccionada).get('nombre'));
		email.setValue(listado1.getStore().getAt(filaseleccionada).get('emil'));
		codigo.setValue(listado1.getStore().getAt(filaseleccionada).get('cod'));		 
		Prov.setValue(listado1.getStore().getAt(filaseleccionada).get('Pro'));
		Munc.setValue(listado1.getStore().getAt(filaseleccionada).get('mun'));
		Dept.setValue(listado1.getStore().getAt(filaseleccionada).get('dep'));		
		equipo.setValue(listado1.getStore().getAt(filaseleccionada).get('equ'));
		RPA.setValue(listado1.getStore().getAt(filaseleccionada).get('RPA'));
		idR.setValue(listado1.getStore().getAt(filaseleccionada).get('id')); 					
		},
	Alta1:function(){		
	idR1.setValue(listado1.getStore().getAt(filaseleccionada).get('id')); 		
	coment=new Ext.form.HtmlEditor({fieldLabel : "Comentario Tecnico",anchor:'95%',bodyStyle:'padding:5px 5px 0',name : "Coment",hiddenName : "Coment",height: 150});
	 this.formulario = new Ext.form.FormPanel({
						frame:true,
						layout: 'form',
			...
			layout: 'form',width: 710,frame:true,height:228,
			items:[this.formulario]
			,bbar: ['->',{text:'Enviar Datos', scope:this,iconCls:'acept',handler:this.AltaEnvio1},
					{text:'Cancelar Envio', scope:this,iconCls:'user_del',handler:this.reset}]	
			});
			formulario1=this.formulario;
			mask.show(); 
	...
	comenta=new Ext.form.HtmlEditor({fieldLabel : "Comentario Tecnico",anchor:'95%',bodyStyle:'padding:5px 5px 0',name : "Comenta",hiddenName : "Comenta",height: 150});
	 ...
	 this.formulario = new Ext.form.FormPanel({
			...
			layout: 'form',width: 710,frame:true,height:228,
			items:[this.formulario]
			,bbar: ['->',{text:'Enviar Datos', scope:this,iconCls:'acept',handler:this.BajaEnvio1},
					{text:'Cancelar Envio', scope:this,iconCls:'user_del',handler:this.reset}]	
			});
			formulario1=this.formulario;
			mask.show();  
	...
	reset:function(){this.form.getForm().reset();window.location = 'tecnico.php?termino=Reportes';},
	reset1:function(){formulario1.getForm().reset();window.location = 'tecnico.php?termino=Reportes';}	
}
