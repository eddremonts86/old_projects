    ...
    this.Render = function(Panel){
        this._btn_add = Ext.create('Ext.Button',{
            ...
            handler:function( This, eOpts){
                ...
                var mydd = Ext.getCmp('prenom_grid').getStore();
                ...
                {
                    for (var i = 0; i < mydd.data.length; i++){
                        if(mydd.data.items[i].data.id_prenomina != "" && mydd.data.items[i].data.id_prenomina != null)
                        {_array.push(mydd.data.items[i].data);
                   ...
                   // console.log(_array);
                    ...
                    var fnCallBack = function()   {
                        var _result_bus = 0; // App.PerformSyncServerRequest('Nomina.Nomina.Buscar2',{mes:mes,anno:anno});
                        if(_result_bus.results > 0){
                            var fnCallBack_after = function()   {
                                App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
                                var _result = App.PerformSyncServerRequest('Nomina.Nomina.Add',{data: Ext.encode(_array),mes:mes,anno:anno});
                                App.HideMsgBox();
                                if(_result)
                                {
        ...
        this._btn_mod = Ext.create('Ext.Button', {
            ...
            handler: function(){
                ...
                var anno= Ext.getCmp('anno').getValue();
                if(mes!='' && anno !=''&& mes!=null && anno !=null){
                this.__data_store_datos = App.BuildJsonStore('Nomina.Nomina.info', {
        ...
        this._btn_mod_inc = Ext.create('Ext.Button', {
            ...
            handler: function(){
                ...
                var anno= Ext.getCmp('anno').getValue();
                if(mes!='' && anno !=''&& mes!=null && anno !=null){
                this.__data_store_datos = App.BuildJsonStore('Nomina.Nomina.info_inc', {
                    fields: [{name: 'datos',header:''}],
                    groupField: 'area',
                    params:{mes:mes,anno:anno},
        ...
        tbar.add({
            ...
            listeners: {
                change:function(This, newValue){
                    if (newValue==false){
                        var result = App.PerformSyncServerRequest('Nomina.Nomina.CargarDatos',{start:0,limit:25});
        ...
        tbar.add({
            ...
            listeners: {
                change:function(){
                    ...
                    var result = App.PerformSyncServerRequest('Nomina.Nomina.CargarAreas',{value:value});
                    this.__data_store.loadData(result.rows);
                    var resultUnidad = App.PerformSyncServerRequest('Nomina.Nomina.Tomar_datos',{combo_value_id:combo_value_id, start:0,limit:25});
        ...
        tbar.add({
            ...
            listeners: {
                change:function(){
                    ...
                    var resultUnidadF = App.PerformSyncServerRequest('Nomina.Nomina.Tomar_datos',{combo_value_id:combo_value_id,combo_area_id:combo_area_id, start:0,limit:25});
...
App.RegisterFunction('NominaGestionar', new NominaGestionar());
