...
//==============================Fluctuacion de a BD==================================================================
    ...
    public function bajas_categorias(&$params)
    {
        ...
        $_start = ($params['start']) ? $params['start'] : 0;
        $_limit = $params['limit'];
        $_table = $this->_dbase->GetTable('vistas_resumen.bajas_categorias ');
        $_data = $_table->GetAll();
        if (is_null($_data))
            return false;
        $_count = count($_data);
    ...
    public function ImpListadoRopaTrabajadores(&$params)
    {
        ...
        $html .= "<div class='contenedor'><table cellspacing='0' cellpadding='0' border='0' class='main'>
                    <tr>
                        ...
                        <td width='100' align='center' class='header'>Pantalon o Salla</td>
...
//        print_r($_data);die;
        $html = "<html><head><style type='text/css'>
                                            .contenedor:after{
                                                ...
                                                border-right:1px solid #000;
                                                color: #FFFFFF;
                                                content: 'Trabajadores segun antigüedad ';
                                                font-size: 12px;
                                                font-weight: bold;
                                                left: -1px;
                                                padding: 3px 7px;
...
//                            $html .= '<td><div style="border: 0;width: 70px; height: 70px; background: url(\'file://'.$_SESSION['APP_PATH'].'/SubSystems/Plantilla/Config/Client/icons/user3.png\') no-repeat;">&nbsp;</div></td>';
    ...
    public function  Validateespecialidades(&$params){
        return true;
    }
    public function  especialidades(&$params){

        $_data = array();
        $_count = 0;
        $_start = ($params['start']) ? $params['start'] : 0;
        $_limit = $params['limit'];
        $_table = $this->_dbase->GetTable('vistas_resumen.trbajador_x_especialidad');
        $_data = $_table->GetAll();
        if (is_null($_data))
            return false;
        $_count = count($_data);
    ...
    public function  ValidateespecialidadesFisicas(&$params){
        return true;
    }
    public function  especialidadesFisicas(&$params){
    ...
    public function  Validatecontespecialidades(&$params){
        return true;
    }
    public function  contespecialidades(&$params){

        $_data = array();
        $_count = 0;
        $_start = ($params['start']) ? $params['start'] : 0;
        $_limit = $params['limit'];
        $_table = $this->_dbase->GetTable('vistas_resumen.especialidad_x_trbajadores ');
        $_data = $_table->GetRange($_limit,$_start);
        if (is_null($_data))
            return false;
        $_count = $_table->GetRowsCount();
    ...
    public function Validatecurso_solicitados(&$params){return true;}
    public function curso_solicitados(&$params){
        $_data = array();
        $_data2 = array();
        $_count = 0;
        $_start = ($params['start']) ? $params['start'] : 0;
        $_limit = $params['limit'];
        $_table = $this->_dbase->GetTable('vistas_resumen.cursos_solicitados');
        $_data = $_table->GetRange($_limit, $_start);
        if (is_null($_data))
            return false;
        $_count = $_table->GetRowsCount();
    ...
    // ===================Listado de trabajadores a Jubilarce===========================================================//
    public function Validatejubilacion(&$params)
    {
        return true;
    }

    public function jubilacion(&$params)
    {
        $_data = array();
        $_count = 0;
        $_start = ($params['start']) ? $params['start'] : 0;
        $_limit = $params['limit'];
        $_table = $this->_dbase->GetTable('vistas_resumen.plantilla_jubilacion');
        $_data = $_table->GetAll();
        $count = $_table->GetRowsCount();
        $result = array('success' => true, 'results' => $count, 'rows' => $_data);
        return $result;
    }
