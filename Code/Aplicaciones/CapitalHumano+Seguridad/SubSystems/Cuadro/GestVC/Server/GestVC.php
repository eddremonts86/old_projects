...
class GestVC extends Module {
    ...
    public function CargarDatos(&$params) {
        ...
        if($causas==1){
            $_table = $this->_dbase->GetTable('vistas_resumen.cuadros_violaciones');
            $_data = $_table->GetRange($_limit, $_start,"($annos = EXTRACT (YEAR FROM fecha_re)) AND ($meses = EXTRACT (MONTH FROM fecha_re))");
        }
        else if($causas==2){
            $_table = $this->_dbase->GetTable('vistas_resumen.cuadro_controles');
            $_data = $_table->GetRange($_limit, $_start,"($annos = EXTRACT (YEAR FROM fecha)) AND ($meses = EXTRACT (MONTH FROM fecha))");
