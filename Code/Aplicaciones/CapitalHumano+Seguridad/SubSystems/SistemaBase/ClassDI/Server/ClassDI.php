...
class ClassDI extends Module {
    ...
    public function ValidateEliminar(&$params) {
        ...
        $count = $datos->Contains(array('clasificacion_d_iid' => "'$id'"));
        if ($count > 0) {
            $this->RegisterError('Operación no válida', "La Clasificación a eliminar está siendo usado por Cargo.");
            return false;
        }
        return true;
    }
    public function Eliminar(&$params) {
        ...
        $result=null;
            if ($id) {
                $datos=$this->_dbase->GetTable("nomencladores.clasificacion_d_i");
    ...
    public function ValidateAdd(&$params){
        ...
        if (!$_valid) {
            $this->RegisterError('Parámetro incorrecto', "Nombre de la Clasificación incorrecto.");
            return false;
        }
        ...
        $actividades = $this->_dbase->GetTable('nomencladores.clasificacion_d_i');
        $count = $actividades->Contains(array('noambre' => "'$nombre'"));

        if ($count < 0)
            return false;

        elseif ($count > 0) {
            $this->RegisterError('Operación no válida', "La Clasificación '$nombre' ya existe.");
            return false;
        }
        return true;
       }
    public function Add(&$params){
       ...
       $datos=$this->_dbase->GetTable("nomencladores.clasificacion_d_i");
       return $data=$datos->InsertValues(array('noambre' => "'$name'"));
    ...
    public function Modid(&$params){
        ...
        $datos=$this->_dbase->GetTable("nomencladores.clasificacion_d_i");
        $datos->Update(array('noambre' => "'$name'"),"id = '$ids'");
        return true;
    }
