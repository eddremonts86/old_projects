...
AdminUsers.prototype.OnModifyPass = function(){
     ...
     {
          title : 'Modificar contraceña del usuario',
...
AdminUsers.prototype.ModifyUserPass = function(){
     if(!Ext.getCmp('_change_user_pass_panel_id').getForm().isValid())
         App.InfoMessage('Información', 'Debe llenar todos los campos,recurede que la nueva contraseña debe coincidir con la confirmación');
    else{
     ...
     if(result)
         ...
         App.InfoMessage('Información', 'Se a cambiado la contraceña correctamente');
