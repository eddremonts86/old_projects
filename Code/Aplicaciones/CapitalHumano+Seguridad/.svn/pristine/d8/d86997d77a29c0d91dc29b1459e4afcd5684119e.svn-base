...
//        ],
        items: [
            ...
            {
                id: 'card-8',
                items:{xtype: 'htmleditor',height:350,width:'100%',id:'Esperiencia',name:'Esperiencia',value:'<h3>Esperiencia Profesional</h3>'}
            },
            {
                id: 'card-9',
...
Cargo.prototype.Add = function(_paccion){
    if (Ext.getCmp('panel').getForm().isValid())
    {
        ...
        var   _result = App.PerformSyncServerRequest('Cargo.Cargo.Add',this.paneladdcargo.getForm().getValues());
...
Cargo.prototype.Modid = function(_paccion,_selectionModel,_selected_rcd){
    ...
    {
        App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
        var  _result = App.PerformSyncServerRequest('Cargo.Cargo.Modif',this.paneladdcargo.getForm().getValues());
        this.__data_store_car.load();
        App.HideMsgBox();
