...
class CuadroNivel extends Module {
    ...
    public function CargarDatos(&$params) {
        ...
        $_limit = $params['limit'];
        $_table = $this->_dbase->GetTable('vistas_resumen.nivels_cuadros');
        $_data = $_table->GetRange($_limit, $_start,"cuadro='true'");
        if (is_null($_data))
            return false;
        $_count = count($_data);
        if ($_count == -1)
    ...
    public function Add(&$params){
       ...
       $datos=$this->_dbase->GetTable("cuadro.niveles");
       for($i=0;$i<count($data);$i++)
       {
           ...
           if(isset($id)){
               ...
               $datosIns=$datos->InsertValues(array(  'trabajador_id' => "'$trabajador_id'",
                                                       'cargo_id' => "'$cargo_id'",
                                                       'nivel' => "'$nivel'"
                                             ));
           }
