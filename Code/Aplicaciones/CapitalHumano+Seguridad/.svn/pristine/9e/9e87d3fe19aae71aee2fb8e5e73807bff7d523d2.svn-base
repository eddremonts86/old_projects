...
function Plan(){
    ...
    this.BuildMainPanel = function(filterObj)
    {
        ...
        var capitulos_plan = Ext.create('Ext.grid.Panel', {
            title: 'Capitulos del plan',
            store: this.__data_store_cap,
            id:'capitulos_creaados',
            name:'capitulos_creaados',columnLines: true,
            border:2,
            autoScroll:true,
            columns: [ { text: 'Capitulo', dataIndex: 'cap_plan', flex: 1 }],
            height: 360,
            width: '100%',
            tools:[{
                    type:'refresh',
                        ...
                        handler:function(){
                            ...
                            var fnCallBack = function() {
                                ...
                                if(result)
                                {
                                    ...
                                    App.InfoMessage('Información', 'Se elimino satisfactoriamente el capitulo');
                                }
                                else
                                    App.InfoMessage('Información', 'Ocurrio un error');
                            }
                            App.ConfirmMessage(fnCallBack, '¿Está seguro que desea eliminar  el capitulo "'+cap_plan+'"?');
                            ...
                            handler:function(This){
                                ...
                                if(id == null || id==''){
                                    ...
                                    var fnCallBack = function()   {
                                        ...
                                        {
                                            This.up('grid').store.remove(record);
                                            App.InfoMessage('Información', 'Ha eliminado una acción del capitulo <b>"'+cap_plan+'"</b>');
                                        }
                                        else
                                        {
                                            Ext.getCmp('cap'+area).getStore().load({area: area})
            ...
            items: [
                    {
                        ...
                        id: 'cap',
                        width:'100%',
                        emptyText:'Nombre del nuevo capitulo',
                        allowBlank: false
                    },
                    {
                        xtype: 'button',
