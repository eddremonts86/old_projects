...
require_once(realpath(dirname(__FILE__)) . '/ErrorManager.php');
...
class Validator
{
     ...
     * @Param Type array [names_array] : Array with the name of expected parameters
     * @Param Type array [params] : Array of parameters
     * @Return mixed : Retorna verdadero, si los parámetros esperados están definidos y tienen una valor
     * distinto de nulo; de lo contrario retorna el primer parámetro encontrado, no defino o de valor nulo.
    **/
    public static function ParamsExist($names_array, &$params)
    {
        $i = 0;
        $cant = count($names_array);

        while ($i < $cant && !isset($params[$names_array[$i]]))
            $i++;

        if ($i < $cant)
            $result = $names_array[$i];
        else
            $result = true;
     ...
     * @Param Type array [names_array] : Array with the name of expected parameters
     * @Param Type array [params] : Array of parameters
     * @Return mixed : Retorna falso, si los parámetros esperados tienen un valor no vacío y diferente de cero;
     * de lo contrario retorna el primer parámetro encontrado, de valor vacío o igual a cero.
    **/
    public static function EmptyParamsExist($names_array, &$params)
    {
        $i = 0;
        $cant = count($names_array);

        while ($i < $cant && !empty($params[$names_array[$i]]))
            $i++;
