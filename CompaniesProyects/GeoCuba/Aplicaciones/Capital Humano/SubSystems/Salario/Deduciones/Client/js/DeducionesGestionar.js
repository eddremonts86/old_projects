function DeducionesGestionar()
{
    ...
    this.Render = function(Panel){
        ...
        var tbar = Ext.getCmp('Deduciones_tbar_id');
        tbar.add('->');
        tbar.add({
            xtype: 'checkbox',
            id: 'select_unidad',
           ...
           // boxLabel: 'Empresa',
            ...
            listeners: {
                change:function(This, newValue){
                    if (newValue==false){
                        var result = App.PerformSyncServerRequest('Deduciones.Deduciones.CargarDatos',{start:0,limit:25});
                        this._store_combo.loadData(result.rows);
                        Ext.getCmp('combo_value_id').enable();
                    }
                    else{
                        Ext.getCmp('Deduciones_grid_id').store.clearData( Ext.getCmp('Deduciones_grid_id').store);
        ...
        tbar.add({
            ...
            listeners: {
                change:function(){
                    ...
                    var result = App.PerformSyncServerRequest('Deduciones.Deduciones.CargarAreas',{value:value});
                    this.__data_store.loadData(result.rows);
                    var resultUnidad = App.PerformSyncServerRequest('Deduciones.Deduciones.Tomar_datos',{combo_value_id:combo_value_id, start:0,limit:25});
                    Ext.getCmp('Deduciones_grid_id').store.loadData(resultUnidad.rows);
        ...
        tbar.add({
            ...
            listeners: {
                change:function(){
                    ...
                    var resultUnidadF = App.PerformSyncServerRequest('Deduciones.Deduciones.Tomar_datos',{combo_value_id:combo_value_id,combo_area_id:combo_area_id, start:0,limit:25});
                    Ext.getCmp('Deduciones_grid_id').store.loadData(resultUnidadF.rows);
...
App.RegisterFunction('DeducionesGestionar', new DeducionesGestionar());
this.panel=null;
this.panelupd=null;
Deduciones.prototype.OnShowWindow = function(_paccion){
    var _value_nom = null;
    if(_paccion == 'add')
    {
...
Deduciones.prototype.OnShowWindow2 = function(_paccion){
    var _value_nom_oll = null;
    var id_tpo=null;
    var _selected_rcd = Ext.getCmp('Deduciones_grid_id').getSelectionModel().getLastSelected();
...
Deduciones.prototype.Add = function(_paccion){
    _result = App.PerformSyncServerRequest('Deduciones.Deduciones.Add',this.panel.getForm().getValues());
    this.__data_store.load({params:{start:0,limit:25}});
}
Deduciones.prototype.Modid = function(_paccion){
    _result = App.PerformSyncServerRequest('Deduciones.Deduciones.Modid',this.panelupd.getForm().getValues());
    this.__data_store.load({params:{start:0,limit:25}});
 }
Deduciones.prototype.deltect=function(_paccion){
    var _selected_rcd = Ext.getCmp('Deduciones_grid_id').getSelectionModel().getLastSelected();
    var id = _selected_rcd.data.id;
    _result = App.PerformSyncServerRequest('Deduciones.Deduciones.Eliminar', {id: id});
    this.__data_store.load({params:{start:0,limit:25}});
}
