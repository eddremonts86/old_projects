function EvaluacionesGestionar()
{
    this._btn_add = null;
    this._btn_mod = null;
    this._btn_del = null;    
    this.Render = function(Panel){
        this._btn_add = Ext.create('Ext.Button',{
            text: 'Guardar',
            id: 'add_Evaluaciones__bt_id',
            tooltip : 'Adiciona un  nuevo Evaluaciones',
            iconCls: 'list_add16',
            handler: Ext.Function.bind(this.Owner.OnShowWindowcp, this.Owner,['add'])
        });
        this._btn_del = Ext.create('Ext.Button',{
            text: 'Eliminar',
            id: 'del_Evaluaciones_btn_id',
            tooltip : 'Elimina el Evaluaciones seleccionado',
        ...
        this._btn_mod = Ext.create('Ext.Button', {
            text: 'Historial',
            id: 'mod_Evaluaciones_btn_id',
            tooltip : 'Modifica el Evaluaciones seleccionado',
            iconCls: 'apply16',
            disabled: false,
            handler: Ext.Function.bind(this.Owner.OnShowWindow2cp, this.Owner, ['upd'])
        });
        var tbar = Ext.getCmp('Evaluaciones_tbar_id');
        tbar.add('->');
...
App.RegisterFunction('EvaluacionesGestionar', new EvaluacionesGestionar());
this.panel=null;
this.panelupd=null;
Evaluaciones.prototype.OnShowWindowcp = function(_paccion){
    var _value_nom = null;
    if(_paccion == 'add')
    {
        var storeM = Ext.getCmp('Evaluaciones_grid_id').store;
        for (var i = 0; i < storeM.count(); i++) {
            ...
            Ext.getCmp('Evaluaciones_grid_id').getView().refresh();
        }
            var mydd =new Array();
            _array = new Array();
            var mydd = Ext.getCmp('Evaluaciones_grid_id').getStore().getModifiedRecords();
            if (mydd.length === 0)
            {
            ...
            else
            {
                ...
                var formada = 0;
                var mystore = Ext.getCmp('Evaluaciones_grid_id').store;
               ...
               var fnCallBack_after = function()   {
                            ...
                            //console.log(formada)
                           var _result = App.PerformSyncServerRequest('Evaluaciones.Evaluaciones.Add',{data: Ext.encode(_array),formada:formada});
                            App.HideMsgBox();
                            if(_result)
                            {
...
Evaluaciones.prototype.OnShowWindow2cp = function(_paccion){
    var _selectionModel = Ext.getCmp('grid').getSelectionModel();
    var _selected_rcd = _selectionModel.getLastSelected();
    var id_tpo=_selected_rcd.data.id;
    if(_paccion == 'upd')
     {
         this.__data_store_historia = App.BuildJsonStore('Evaluaciones.Evaluaciones.CargarDatos_hist',{
             fields: [
                 {name: 'nombre'},
                 {name: 'cg'},
                 {name: 'ce'},
         ...
         this._grid = new Ext.grid.Panel({
             id: 'Evaluaciones_grid_id_historia',
             name: 'Evaluaciones_grid_id_historia',
...
Evaluaciones.prototype.Add = function(_paccion){

    if (Ext.getCmp('panel').getForm().isValid())
    {
        App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
        var   _result = App.PerformSyncServerRequest('Evaluaciones.Evaluaciones.Add',this.panel.getForm().getValues());
        ...
        if(_result)
        {
            ...
            this.__data_store.load();
            App.InfoMessage('Información', 'Evaluaciones adicionado satisfactoriamente');
        }
    }
}
Evaluaciones.prototype.Modid = function(_paccion,_selectionModel,_selected_rcd){

    if (Ext.getCmp('panelupd').getForm().isValid())
    {
        App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
        var _result = App.PerformSyncServerRequest('Evaluaciones.Evaluaciones.Modid',this.panelupd.getForm().getValues());
        this.__data_store.load();
        App.HideMsgBox();
        ...
        if(_result)
        {
            ...
            this.__data_store.load();
            App.InfoMessage('Información', 'Evaluaciones modificado satisfactoriamente');
        }
    }
 }
Evaluaciones.prototype.deltectcp=function(_paccion){
    var me = this;
    var fnCallBack = function() {
        var _selected_rcd = Ext.getCmp('Evaluaciones_grid_id').getSelectionModel().getLastSelected();
        var id = _selected_rcd.data.id;
        App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
        _result = App.PerformSyncServerRequest('Evaluaciones.Evaluaciones.Eliminar', {id: id});
        App.HideMsgBox();

        if(_result)
        {
            me.__data_store.load();
            App.InfoMessage('Información', 'Evaluaciones eliminado correctamente');
        }
    }

    App.ConfirmMessage(fnCallBack, '¿Está seguro que desea eliminar este Evaluaciones?');
