function GestVC(){
    ...
    this.Init = function()
    {
        ...
        App.InsertMenuItem('Gestion de Cuadros',_menu_item_config_GestVC);
    ...
    this.BuildMainPanel = function(filterObj)
    {
        ...
        this.__data_store_niveles = App.BuildJsonStore('GestVC.GestVC.niveles',{
            fields: [
                {name: 'id'},
                {name: 'nombre'}
            ],
        ...
        this.__data_store_cuadros = App.BuildJsonStore('GestVC.GestVC.CargarDatoscuadros',{
            fields: [
                {name: 'id'},
                {name: 'nombre'}
            ],
                ...
                selectionchange: {
                    fn: function(View, selections, options) {
                        if(selections.length == 0){
                            this.Disable('GestVCGestionar');
                        }
                        else
                        {
                          this.Enable('GestVCGestionar');
                        }
        ...
        var panel_datos=Ext.create('Ext.form.Panel', {
            title: 'Otorgar GestVC ',
            bodyPadding: 5,
            width: 350,
            url: 'save-form.php',
            layout: 'anchor',
            id:'cuadros_form_GestVC',
            name:'cuadros_form_GestVC',
            region: 'east',
            defaults: {anchor: '100%'},
           // defaultType: 'textfield',
            items: [cuadro,{
                ...
                name: 'from_date_Ractivación_GestVC',
                id: 'from_date_Ractivación_GestVC',
            ...
            buttons: [
                ...
                handler: function() {
                    ...
                    var _selectionModel = Ext.getCmp('GestVC_grid_id').getSelectionModel();
                    var _selected_rcd = _selectionModel.getLastSelected();
                    var id =_selected_rcd.data.id;
                    if (form.isValid() && id !='') {
                        var mensaje=Ext.getCmp('message_GestVC').getValue();
                        var fecha=Ext.getCmp('from_date_Ractivación_GestVC').getValue();
                        var cuadro=Ext.getCmp('Cuadros_GestVC').getValue();
                        var fnCallBack = function() {
                            App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
                            var   _result = App.PerformSyncServerRequest('GestVC.GestVC.Add_control',{id:id,cuadro:cuadro,fecha:fecha,mensaje:mensaje});
                            App.HideMsgBox();
                            if(_result)
                            {
                                form.reset();
                                Ext.getCmp('Cuadros_GestVC').focus(true, true);
...
App.RegisterModule('GestVC', new GestVC());
