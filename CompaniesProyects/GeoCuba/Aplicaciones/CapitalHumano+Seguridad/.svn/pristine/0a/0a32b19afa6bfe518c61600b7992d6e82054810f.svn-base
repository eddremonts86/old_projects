...
class NPreparacion extends Module {
    ...
    public function ValidateEliminar(&$params) {
        ...
        $count = $datos->Contains(array('nivel_preparacionid' => "'$id'"));
        if ($count > 0) {
            $this->RegisterError('Operación no válida', "El Nivel de Preparación a eliminar está siendo usado por Cargo.");
            return false;
        }
    ...
    public function Eliminar(&$params) {
        ...
        $result=null;
            if ($id) {
                $datos=$this->_dbase->GetTable("nomencladores.nivel_preparacion");
    ...
    public function ValidateAdd(&$params){
        ...
        if (!$_valid) {
            $this->RegisterError('Parámetro incorrecto', "Nivel de Preparación incorrecto.");
            return false;
        }
        ...
        $actividades = $this->_dbase->GetTable('nomencladores.nivel_preparacion');
        ...
        elseif ($count > 0) {
            $this->RegisterError('Operación no válida', "El  Nivel de Preparación '$nombre' ya existe.");
            return false;
        }return true;
       }
