    ...
    this.Render = function(Panel){
        this._btn_add = Ext.create('Ext.Button',{
            ...
            handler:function(){
                ...
                Ext.getCmp('Descuentos_grid_id').store.add(record);
            }
        });
        this._btn_del = Ext.create('Ext.Button',{
            text: 'Eliminar',
            id: 'del_Descuentos_btn_id',
        ...
        var tbar = Ext.getCmp('Descuentos_tbar_id');
        ...
        tbar.add({
            ...
            listeners: {
                change:function(){
                    console.log(Ext.getCmp('Descuentos_grid_id').store);
                    Ext.getCmp('Descuentos_grid_id').store.clearFilter();
                    Ext.getCmp('Descuentos_grid_id').store.filter("nombre", Ext.getCmp('searchField_class_cien').getValue());
                },scope:this
            }
        });*/
    }
...
App.RegisterFunction('DescuentosGestionar', new DescuentosGestionar());
this.panel=null;
this.panelupd=null;
Descuentos.prototype.OnShowWindowcp = function(_paccion){
    var _value_nom = null;
    if(_paccion == 'add')
    {
...
Descuentos.prototype.OnShowWindow2cp = function(_paccion){
    var _value_nom_oll = null;
    var id_tpo=null;
    var _selectionModel = Ext.getCmp('Descuentos_grid_id').getSelectionModel();
...
Descuentos.prototype.Add = function(_paccion){

    if (Ext.getCmp('panel').getForm().isValid())
    {
        App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
        var   _result = App.PerformSyncServerRequest('Descuentos.Descuentos.Add',this.panel.getForm().getValues());
...
Descuentos.prototype.Modid = function(_paccion,_selectionModel,_selected_rcd){

    if (Ext.getCmp('panelupd').getForm().isValid())
    {
        App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
        var _result = App.PerformSyncServerRequest('Descuentos.Descuentos.Modid',this.panelupd.getForm().getValues());
        this.__data_store.load();
        App.HideMsgBox();
        ...
        if(_result)
        {
            ...
            this.__data_store.load();
            App.InfoMessage('Informaci&oacute;n', 'Descuentos modificado satisfactoriamente');
        }
    }
 }
Descuentos.prototype.deltectcp=function(_paccion){
    var me = this;
    var fnCallBack = function() {
        var _selected_rcd = Ext.getCmp('Descuentos_grid_id').getSelectionModel().getLastSelected();
        var id = _selected_rcd.data.id;
        App.ShowMsgBox(Ext.Msg.wait('Por favor, espere un momento', 'Procesando...'));
        _result = App.PerformSyncServerRequest('Descuentos.Descuentos.Eliminar', {id: id});
        App.HideMsgBox();

        if(_result)
        {
            me.__data_store.load();
            App.InfoMessage('Informaci&oacute;n', 'Descuentos eliminado correctamente');
        }
    }
