...
function mosGetLink( $mitem, $level, $class_sfx='' ) {
    ...
    if ($mitem->cnt > 0) {
		   if ($level == 0) {
                ...
                $txt = "<span><span><a class=\"sub\" target=\"_window\"  href=\"$mitem->link\">$mitem->name</a></span></span>";
		   }
		} else {
    ...
    if ($mitem->cnt > 0) {
				if ($level == 0) {
                $txt = "<span><span><a href=\"#\" class=\"main\" onClick=\"javascript: window.open('$mitem->link', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550');\" class=\"$menuclass\">$mitem->name</a></span></span>\n";
		   					$main = "main";
		} else {
                $txt = "<span><span><a href=\"#\" class=\"sub\" onClick=\"javascript: window.open('$mitem->link', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550');\" class=\"$menuclass\">$mitem->name</a></span></span>\n";
		   }
		} else {
		    $txt = "<span><span><a href=\"#\" onClick=\"javascript: window.open('$mitem->link', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550');\" class=\"$menuclass\">$mitem->name</a></span></span>\n";
		}
  ...
  if ($mitem->cnt > 0) {
		...
		$random = (rand()%30000000000000);
		    if ($level == 0) {
                $txt = "<span class=\"s5_outer_active\"><span class=\"s5_outer\"><a class=\"active\" href=\"$mitem->link\">$mitem->name</a></span></span>";
								$main = "main";
		   } else {
...
function mosShowListMenu($menutype) {
	...
	$indents = array(
			...
			array( "\t\t<ul><li class='s5_menutop'></li>", "\t\t\t<li class='noback'>" , '</li>', "\t\t<li class='s5_menubottom'></li></ul>" ),
			array( "\t\t<ul><div class='s5_menutop'></div>", "\t\t\t<li>" , '</li>', "\t\t<li class='s5_menubottom'></li></ul>" ),
			array( "\t\t\t<ul><div class='s5_menutop'></div>", "\t\t\t\t<li>" , '</li>', "\t\t\t<li class='s5_menubottom'></li></ul>" ),
			array( "\t\t\t\t<ul><div class='s5_menutop'></div>", "\t\t\t\t\t<li>" , '</li>', "\t\t\t\t<li class='s5_menubottom'></li></ul>" ),
	);
	$children = array();
